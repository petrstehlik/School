                                     ...
                                     int resolution_bits )
{
  ...
  printf("comp data len: %d\n", compressed_data_length);

  for (int i = 0; i < compressed_data_length; i++) {
  ...
  for (int i = 0; i < uncompressed_data_length; i++) {
	printf("%x (%d) ", uncompressed_data[i], uncompressed_data[i]);
  }
  if ( compressed_data )
...
static void process_gif_stream( int gif_file )
{
  ...
  printf("color resolution bits %d\n", color_resolution_bits);
  ...
  if ( screen_descriptor.fields & 0x80 )
  {
    ...
    for (int i = 0; i < global_color_table_size; i++) {
		printf("%d (%d, %d, %d)\n", i, global_color_table[i].r, global_color_table[i].g, global_color_table[i].b);
    }
  }
